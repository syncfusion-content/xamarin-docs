...
`SfDataGrid` supports selection via keyboard interaction for all the Xamarin.Forms platforms including Xamarin.Forms.macOS and Xamarin.Forms.WPF platforms.In Xamarin.Forms.Android and Xamarin.Forms.iOS platform datagrid supports Keyboard interaction via external keyboard which attached to the devices.Keyboard interaction will not have any effect when the [SfDataGrid.SelectionMode](https://help.syncfusion.com/cr/xamarin/Syncfusion.SfDataGrid.XForms.SfDataGrid.html#Syncfusion_SfDataGrid_XForms_SfDataGrid_SelectionMode) is set as `SelectionMode.None`.
