...
* `Vertical` - When `TokenWrapMode` is set to `Vertical` the selected items will be wrap to the next line of the SfAutoComplete.

* `None` - When `TokenWrapMode` is set to `None` the selected item will be wrap in horizontal orientation.
...
When selecting the multiple items, the selected items can be divided with a desired character given for a delimiter.
