...
DataSource also allows to sort the bound source by using the [DataSource.SortDescriptors](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.DataSource~SortDescriptors.html) property.  You can create a [SortDescriptor](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.SortDescriptor.html) for the property to be sorted and add it in the `DataSource.SortDescriptors` collection. 
...
DataSource also allows to sort the bound source by using the [DataSource.GroupDescriptors](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.DataSource~GroupDescriptors.html) property. You can create a [GroupDescriptor](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.GroupDescriptor.html) for the property to be grouped and add it in the `DataSource.GroupDescriptors` collection. 
...
`DataSource` listens to the manipulation operations such as add, delete and data update (property change) at runtime and responds based on the [LiveDataUpdateMode](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.LiveDataUpdateMode.html) property. The default value is `LiveDataUpdateMode.Default`. `LiveDataUpdateMode` property holds the following two enumeration values:

* Default - This updates only the corresponding underlying data when the property is changed. 
* AllowDataShaping - This updates corresponding underlying data. Also, it reshapes the data based on sorting, filtering, grouping when the property is changed. 
...
When the [Source](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.DataSource~Source.html) property bind with different types of underlying collection derived from the same type, you need to set [SourceType](https://help.syncfusion.com/cr/cref_files/xamarin/datasource/Syncfusion.DataSource.Portable~Syncfusion.DataSource.DataSource~SourceType.html) as base type for all different types.
