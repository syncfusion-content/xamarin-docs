...
SfDataGrid provides interactive support to manipulate data using the [SfDataPager](http://help.syncfusion.com/cr/cref_files/xamarin/sfdatagrid/Syncfusion.SfDataGrid.XForms~Syncfusion.SfDataGrid.XForms.DataPager_namespace.html# “”) control. It also provides built-in options to page data on demand when dealing with large volumes of data. It allows to place the `SfDataPager` above or below ,based on the requirement to easily manage the data paging.

To use the paging functionality in SfDataGrid add the following namespace to the project:
...
SfDataGrid performs paging of data using the `SfDataPager`. To enable paging, follow the procedure:
...
SfDatagrid allows to change the appearance by writing a style class overriding from `AppearanceManager` and assigning it to the [SfDataPager.AppearanceManager](http://help.syncfusion.com/cr/cref_files/xamarin/sfdatagrid/Syncfusion.SfDataGrid.XForms~Syncfusion.SfDataGrid.XForms.DataPager.SfDataPager~AppearanceManager.html) property.
