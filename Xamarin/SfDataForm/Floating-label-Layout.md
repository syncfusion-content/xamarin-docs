...
N> To avoid crash while using unsupported editors with `DataForm` layout option as `TextInputLayout` we can set layout options for unsupported editors' datafields using this [link](https://help.syncfusion.com/xamarin/sfdataform/floating-label-layout#changing-layout-options-of-the-dataformitem)
